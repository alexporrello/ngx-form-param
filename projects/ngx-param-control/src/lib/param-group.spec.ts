...
async function setup(initialUrl = '/') {
  TestBed.configureTestingModule({
    ...
    providers: [provideRouter([{ path: '**', component: GroupHostComponent }])],
  });
...
describe('paramGroup()', () => {
  ...
  describe('FormGroup structure', () => {
    it('returns a FormGroup with the correct control names', async () => {
      ...
      const keys = Object.keys(component.filters.controls);
    ...
    it('creates a FormControl for each entry in the config', async () => {
      const { component } = await setup('/');
      expect(component.filters.controls['search']).toBeTruthy();
      expect(component.filters.controls['page']).toBeTruthy();
      expect(component.filters.controls['active']).toBeTruthy();
    });
  });
  ...
  describe('control → URL sync', () => {
    it('updates the URL when a control inside the group changes', fakeAsync(async () => {
      ...
      component.filters.controls['search'].setValue('rxjs');
      tick(0);
    ...
    it('updates the URL independently for each control', fakeAsync(async () => {
      ...
      component.filters.controls['page'].setValue(5);
      tick(0);
      component.filters.controls['active'].setValue(true);
      tick(0);
    ...
    it('removes a param when its value reverts to the default', fakeAsync(async () => {
      ...
      component.filters.controls['page'].setValue(1); // back to default
      tick(0);
    ...
    it('patchValue updates multiple controls simultaneously', fakeAsync(async () => {
      ...
      component.filters.patchValue({ search: 'forms', page: 2 });
      tick(0);
  ...
  describe('URL → control sync', () => {
    it('updates individual controls when the URL changes', fakeAsync(async () => {
      ...
      expect(component.filters.controls['search'].value).toBe('signal');
      expect(component.filters.controls['page'].value).toBe(2);
    }));
    ...
    it('resets controls to defaults when params are removed from URL', fakeAsync(async () => {
      ...
      expect(component.filters.controls['page'].value).toBe(1);
    }));
  });
  ...
  describe('subscription cleanup', () => {
    it('stops updating the URL after the host component is destroyed', fakeAsync(async () => {
      ...
      component.filters.controls['search'].setValue('ghost');
      tick(0);
  ...
  // ── Custom serializers in groups ───────────────────────────────────────────

  describe('custom serializers', () => {
    it('applies a custom serializer registered on a group entry', fakeAsync(async () => {
      const CSV_SERIALIZER = {
      ...
      class CsvHost {
        readonly g = paramGroup({
          tags: { paramName: 'tags', serializer: CSV_SERIALIZER, defaultValue: [], debounceMs: 0 },
        });
      }
      ...
      TestBed.configureTestingModule({
        imports: [CsvHost],
        providers: [provideRouter([{ path: '**', component: CsvHost }])],
      });

      const r = TestBed.inject(Router);
      await r.navigateByUrl('/?tags=a,b,c');
      const fix = TestBed.createComponent(CsvHost);
      fix.detectChanges();

      expect(fix.componentInstance.g.controls['tags'].value).toEqual(['a', 'b', 'c']);

      fix.componentInstance.g.controls['tags'].setValue(['x', 'y']);
      tick(0);
      expect(r.url).toContain('tags=x%2Cy');
    }));
  });
});
